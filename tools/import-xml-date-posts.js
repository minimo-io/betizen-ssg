...
try {
  const importXmlFile = "./tools/betizen-sitemaps/posts-XML Sitemap.xml";
  const postType = "blog"; // must be the post type inside the ./content/{language_code} directory
  ...
  if (xmlStats.isFile()) {
    ...
    for (url of nodes.querySelectorAll("url")) {
      ...
      let fileSlug = location
        ...
        .replace("/blog", "")
        .replace("/casino/", "")
        .replace("/aprender-a-jugar/", "")
        .replace("/noticias/", "")
        .replace("/novidades/", "")
        .replace("/learn/", "")
        .replace("/aprender-a-jogar/", "")
      ...
      if (fileSlug != "" && urlLang == "es") {
        ...
        try {
          ...
          console.error("> ERROR: Could not open file: " + fileSlug);
          process.exit();
        }
      }
